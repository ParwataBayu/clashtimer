...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // normalize response to a predictable shape for clients
    let out: any = response;
    try {
      if (typeof response === 'string') {
        out = { choices: [{ message: { content: response } }] };
      } else if (response && (response as any).choices == null && (response as any).message) {
        out = { choices: [{ message: { content: (response as any).message } }] };
      }
    } catch {
      out = response;
    }
