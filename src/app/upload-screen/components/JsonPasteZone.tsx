...
// Auto-detect type for each item from JSON
function parseCocJsonAuto(raw: string): ParsedItem[] {
  try {
    const data = JSON.parse(raw);
    const results: ParsedItem[] = [];
    let idx = 0;

    // Parse buildings, heroes, traps as "Bangunan"
...
export default function JsonPasteZone({ onParsed, disabled }: JsonPasteZoneProps) {
  ...
  const handleParse = useCallback(() => {
    ...
    const items = parseCocJsonAuto(jsonText);
    if (items.length === 0) {
      setError(
        'Tidak ada upgrade aktif ditemukan. Pastikan JSON memiliki field "timer" untuk bangunan/unit/mantra yang sedang upgrade.'
      );
      return;
    }
    setError(null);
  ...
  return (
    ...
    >
      ...
      <p className="helper-text mb-3">
        Sistem akan otomatis mendeteksi dan memisahkan antara upgrade Bangunan (buildings, heroes, traps) dan Lab (units, spells, siege_machines, pets).
      </p>
